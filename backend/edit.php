...
if ($bID) {
    // get the databse info for just the ID we clicked -need a WHERE clause
    $bIDSql = "SELECT name, bout, koPercent, status, titles, alias, dob, nationality, division, stance, height, reach, image, youtubeURL, star, ranking, win, lose, draw FROM boxer WHERE boxerID = '$bID'";
    $bIDResult = mysqli_query($conn, $bIDSql);

    if (mysqli_num_rows($bIDResult) == 0) {
        $messageUNQ = "Unable to find that record. <br>";
    } else {
        // fetch just one row  - array is by numbers instead of names
        ...
        $alias = $bIDRow[5];
        $dob = $bIDRow[6];
        $nationality = $bIDRow[7];
        $division = $bIDRow[8];
        $stance = $bIDRow[9];
        $height = $bIDRow[10];
        $reach = $bIDRow[11];
        $fileName = $bIDRow[12];
        $youtubeURL = $bIDRow[13];
        $star = $bIDRow[14];
        $ranking = $bIDRow[15];
        $win = $bIDRow[16];
        $lose = $bIDRow[17];
...
if (isset($_POST['submit'])) {
    ...
    $boxerNameNEW = ucwords(trim($_POST['boxerName']));
    $boutNEW = $_POST['bout'];
    $koPercentNEW = $_POST['koPercent'];
    $statusNEW = ucwords(trim($_POST['status']));
    $titlesNEW = $_POST['titles']; //can be null
    $aliasNEW = ucwords(trim($_POST['alias'])); //can be null
    $dobNEW = $_POST['dob'];
    $nationalityNEW = $_POST['countries'];
    $divisionNEW = $_POST['division'];
    $stanceNEW = $_POST['stance'];
    $heightNEW = $_POST['height']; //can be null
    $reachNEW = $_POST['reach']; //can be null
    $youtubeURLNEW = $_POST['youtubeURL']; //can be null
    $starNEW = $_POST['star']; //can be null
    $rankingNEW = $_POST['ranking']; //can be null
    $winNEW = $_POST['win'];
    $loseNEW = $_POST['lose'];
    $drawNEW = $_POST['draw'];
    // Image infomation collection
    $fileNameNEW = $_FILES['boxerPic']['name'];
    $tempNameNEW = $_FILES['boxerPic']['tmp_name'];
    $sizeNEW = $_FILES['boxerPic']['size'];
    $fileTypeNEW = $_FILES['boxerPic']['type'];
    $errorNEW = $_FILES['boxerPic']['error'];



    ////////////////////template//////////////////////////////////////
    $movieNameNEW = ucwords(trim($_POST['movieName']));
    $relDateNEW = $_POST['relDate'];
    $prodCompanyNEW = ucwords(trim($_POST['prodCompany']));
    $prodCompanyURLNEW = trim($_POST['prodCompanyURL']);
    $runTimeNEW = $_POST['runTime'];
    $ratingNEW = strtoupper(trim($_POST['rating']));
    $descriptionNEW = trim($_POST['description']);
    /////////////////////////////////////////////////////////////////
    ...
    //name vals
    if ($boxerName == "" || $boxerNameNEW == "") {
        $errorMessage = "Please enter a boxer's name. <br>";
        $valid = false;
    }
    if (is_numeric($boxerName) == true || is_numeric($boxerNameNEW) == true) {
    ...
    if (is_numeric($bout) == false || is_numeric($boutNEW) == false) {
        $errorMessage .= "Please enter a number for the NUMBER of bouts. <br>";
        $valid = false;
    }

    //KO percent vals
    if ($koPercent == "" || $koPercentNEW == "") {
        $errorMessage .= "Please enter the knockout percentage.";
        $valid = false;
    }
    if (is_numeric($koPercent) == false || is_numeric($koPercentNEW) == false) {
    ...
    if (is_numeric($win) == false || is_numeric($winNEW) == false) {
    ...
    if (is_numeric($lose) == false || is_numeric($loseNEW) == false) {
        $errorMessage .= " Please enter a number for the NUMBER of wins. <br>";
        $valid = false;
    }
    if ($draw == ""  || $drawNEW == "") {
    ...
    if (is_numeric($draw) == false || is_numeric($drawNEW) == false) {
        $errorMessage .= " Please enter a number for the NUMBER of wins. <br>";
        $valid = false;
    }
    ...
    if ($fileNameNEW) {
        if ($fileTypeNEW  != "image/jfif" && $fileTypeNEW  != "image/jpeg" && $fileTypeNEW != "image/pjpeg" && $fileTypeNEW != "image/png") {
            $valid = false;
    ...
    // if ($boxerNameNEW || $boutNEW || $koPercentNEW || $statusNEW  || $titlesNEW ||  $aliasNEW || $dobNEW || $nationalityNEW || $divisionNEW || $stanceNEW || $heightNEW || $reachNEW || $youtubeURLNEW || $starNEW || $rankingNEW || $winNEW || $loseNEW || $drawNEW || $fileNameNEW) {
    if ($valid == true) {
        if ($fileNameNEW) {
            if (move_uploaded_file($tempNameNEW, $original . $fileNameNEW)) {
                $thisFile = $original . basename($fileNameNEW);
                customeSize($thisFile, $thumbnail, 318);
                customeSize($thisFile, $display, 600);

                $updateImageSQL = "UPDATE boxer set
                image = '$fileNameNEW'
                WHERE boxerID = '$bID'";
        ...
        $updateSql = "UPDATE boxer set
        name = '$boxerNameNEW',
        bout = '$boutNEW',
        koPercent = '$koPercentNEW',
        status = '$statusNEW',
        titles = '$titlesNEW',
        alias = '$aliasNEW',
        dob = '$dobNEW',
        ...
        if (mysqli_query($conn, $updateSql)) {
            ...
            $message = "Error. Record not updated. " . mysqli_error($conn) . '<br>';
        }
    }
    // else {
...
<div class="container ">
    ...
    <div class="row">
        <div class="col-5">
            ...
            echo $message;
            $sql = "SELECT name, boxerID FROM boxer";
            $result = mysqli_query($conn, $sql);

            if (mysqli_num_rows($result) == 0) {
                echo "<p><a href=\"insert.php\"> Click here </a> to add a Boxer first </p> ";
            } else {

                echo "<ul class=\"list-group mb-5\">";
                while ($row = mysqli_fetch_assoc($result)) {
                    $boxerNameList = $row['name'];
                    $id = $row['boxerID'];
                    ?>
                    <li class="list-group-item"><a href="edit.php?bID=<?php echo $id; ?>">
                            <?php echo $boxerNameList; ?>
                        </a>
            ...
            <div class="col-6">
                <form method="post" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="boxerName" id="boxerName" value="<?php if ($boxerNameNEW) {
                                                                                                            echo $boxerNameNEW;
                                                                                                        } else {
                                                                                                            echo $boxerName;
                                                                                                        }
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control " name="bout" id="bout" value="<?php if ($boutNEW) {
                                                                                                    echo $boutNEW;
                                                                                                } else {
                                                                                                    echo $bout;
                                                                                                }
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="koPercent" id="koPercent" value="<?php if ($koPercentNEW) {
                                                                                                            echo $koPercentNEW;
                                                                                                        } else {
                                                                                                            echo $koPercent;
                                                                                                        } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="status" id="status">
                            ...
                            <option value="retired" <?php if ($status == "Retired") {
                                                        echo 'selected';
                                                    } ?>>Retired</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="titles" name="titles" rows="3"><?php if ($titlesNEW) {
                                                                                                echo $titlesNEW;
                                                                                            } else {
                                                                                                echo $titles;
                                                                                            } ?></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="alias" id="alias" value="<?php if ($aliasNEW) {
                                                                                                    echo $aliasNEW;
                                                                                                } else {
                                                                                                    echo $alias;
                                                                                                }  ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="dob" id="dob" value="<?php if ($dobNEW) {
                                                                                                echo $dobNEW;
                                                                                            } else {
                                                                                                echo $dob;
                                                                                            } ?>">
                    </div>
                    <!-- when you have time write the code to select for countries Hint
                                                                                                                                                                                                                                                                                                                                                                                           <option if ($nationality == "Active") {
                                                                                                                                                                                                                                                                                                                                                                                                                                echo 'selected'}> -->
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Height in cm" name="height" id="height" value="<?php if ($heightNEW) {
                                                                                                                                echo $heightNEW;
                                                                                                                            } else {
                                                                                                                                echo $height;
                                                                                                                            } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Reach in cm" name="reach" id="reach" value="<?php if ($reachNEW) {
                                                                                                                                echo $reachNEW;
                                                                                                                            } else {
                                                                                                                                echo $reach;
                                                                                                                            }  ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="youtubeURL" id="youtubeURL" value="<?php if ($youtubeURLNEW) {
                                                                                                                echo $youtubeURLNEW;
                                                                                                            } else {
                                                                                                                echo $youtubeURL;
                                                                                                            } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="star" placeholder="Example: 4.5" id="star" value="<?php if ($starNEW) {
                                                                                                                            echo $starNEW;
                                                                                                                        } else {
                                                                                                                            echo $star;
                                                                                                                        } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Example: 35" name="ranking" id="ranking" value="<?php if ($rankingNEW) {
                                                                                                                                    echo $rankingNEW;
                                                                                                                                } else {
                                                                                                                                    echo $ranking;
                                                                                                                                } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="win" id="win" value="<?php if ($winNEW) {
                                                                                                echo $winNEW;
                                                                                            } else {
                                                                                                echo $win;
                                                                                            } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="lose" id="lose" value="<?php if ($loseNEW) {
                                                                                                    echo $loseNEW;
                                                                                                } else {
                                                                                                    echo $lose;
                                                                                                } ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="draw" id="draw" value="<?php if ($drawNEW) {
                                                                                                    echo $drawNEW;
                                                                                                } else {
                                                                                                    echo $draw;
                                                                                                } ?>">
